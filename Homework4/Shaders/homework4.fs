...
uniform sampler2D texture_pineapple;
...
void main(void)                                                   
{
    ...
    vec4 tex_pineapple = texture(texture_pineapple, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + tex_color + tex_color_light + tex_pineapple;
    }
    else if(texture_blend == 1)
    {
        color = tex_color * tex_color_light * tex_pineapple;
    }
